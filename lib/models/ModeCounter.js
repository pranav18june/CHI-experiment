 ...
 *   2. Latin-Square Schedule counts (s0 to s7) independently within each expertise group.
 *   3. A monotonic assignment sequence per group, incremented atomically. The
 *      sequence — not the counts — decides the cell (see lib/assignment.js), so
 *      concurrent starts cannot race on a stale snapshot.
 *
...
const ModeCounterSchema = new mongoose.Schema(
  {
    ...
    novice: {
      // Atomic assignment sequence — the authority for cell allocation.
      // The c*/s* counts below are descriptive only (admin dashboard).
